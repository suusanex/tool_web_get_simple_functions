...
| RC-003 | Required candidate field `venueEvidence` | runtime contract lists candidate fields including `venueEvidence`; implementation-contract output DTO shape includes `venueEvidence` | output DTO shape in implementation-contract includes `venueEvidence` | `ExtractedEventCandidate.cs` has `VenueEvidence`; `AzureOpenAIRestExtractionClient.BuildSchema()` includes `venueEvidence` in required/properties; request test asserts schema contains it | TP-RC003-A, TP-RC003-C | Done | DTO / schema / test evidence are aligned |
...
| RC-003 | Candidate validation: date/source/venue/eventType/evidence | invalid date order, missing/untraceable source URL, unconfirmed venue, unsupported event type, empty evidence are rejected | `EventCandidateValidator` is production binding | `EventCandidateValidator.cs` rejects empty title, invalid date order, unconfirmed venue, untraceable sourceUrls, non-collaboration eventType, empty evidence snippets, and untraceable title/date/venue/evidence values | TP-RC003-C | Done | validator implements deterministic post-LLM checks and tests cover traceable / untraceable cases |
| RC-003 | Candidate validation: values traceable to candidate input | date/title/venue values must be traceable to source text/source URL/detected strings/aliases | implementation-contract requires traceability checks for date/title/venue/source/evidence | `EventCandidateValidator.cs` checks title / venue / evidence against the combined corpus and checks dates against detected strings plus title/text/image alt tokens; tests include image-alt-only date evidence | TP-RC003-C | Done | image alt date evidence is explicitly covered |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| partial | Fake `HttpMessageHandler` / `IHttpClientFactory`, in-memory extraction inputs, and deterministic candidate fixtures are used in tests | Test design expects fake/stub/mock for CI-safe paths | `IHttpClientFactory`, in-memory DTOs | `RakuSpaSourceCollector`, `AzureOpenAIRestExtractionClient`, `EventCandidateValidator`, `EventSelectionService` | `Program.cs` wires the same production concrete services in the Functions app | PartiallyDone | Function entrypoint / Easy Auth / malformed AOAI response paths still need additional targeted tests or manual verification |
...
| TP-RC002-A | RC-002 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/RakuSpaSourceCollectorTests.cs` | Yes | fixture official news HTML produces candidate article URL/title/text in `ExtractionInput` | `CollectAsync_FollowsVisibleSignalLinksAndCampaignLinksAfterFilteringAsync` verifies visible link text based candidate discovery | official page anchor text drives candidate fetch |
| TP-RC002-B | RC-002 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/RakuSpaSourceCollectorTests.cs` | Yes | linked campaign page visible text and image alt included; raw image bytes not included | same fixture test verifies campaign page follow-up and image-alt-derived date strings | extraction remains text-only |
...
| TP-RC002-E | RC-002 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/RakuSpaSourceCollectorTests.cs` | Yes | timeout/page-size/count limits produce explicit error or bounded non-silent behavior | `CollectAsync_WhenPageExceedsLimit_ThrowsAsync` verifies explicit page-size failure; count bounding remains code-inspected | timeout path still needs a dedicated test if selected later |
| TP-RC003-A | RC-003 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/AzureOpenAIRestExtractionClientTests.cs` | Yes | AOAI request contains `response_format.type=json_schema`, strict schema, required fields, `additionalProperties:false` | request builder test asserts `json_schema`, `strict`, schema name, and `venueEvidence`; oversize input test verifies explicit error | request contract is now aligned |
...
| TP-RC003-C | RC-003 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/EventCandidateValidatorTests.cs` | Yes | invalid/untraceable candidate is rejected and warning/error recorded | validator tests cover traceable candidate, venue/date/evidence rejection, and image-alt-only date traceability | deterministic validation has direct test coverage |
| TP-RC003-D | RC-003 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/EventSelectionServiceTests.cs` | Yes | active/next/none/overlap/same-start deterministic selection | selector tests cover overlap warning, same-start next warning, and no-event response | deterministic ordering is directly exercised |
| TP-RC003-E | RC-003 | `tests/ToolWebGetSimpleFunctions.Functions.Tests/Services/EventSelectionServiceTests.cs` | Yes | start/start+1/start+2 initial_peak; start+3 after_initial_peak; before_event; no_event | selector tests cover start+2 inclusive `initial_peak`, short-event capping, and no-event representation via nulls | `after_initial_peak` and `before_event` remain code-inspected in this artifact |
...
| --- | --- | --- | --- | --- |
| VK-GAP-001 | missing-test | Function entrypoint response shape / Easy Auth header guard / date parsing paths still lack direct tests | `coverage-gap-resolution-slice.agent.md` or implementation phase | `src/ToolWebGetSimpleFunctions.Functions/Functions/RakuSpaKandaEventsFunction.cs`; `tests/ToolWebGetSimpleFunctions.Functions.Tests/` |
| VK-GAP-002 | missing-test | AOAI malformed response / HTTP failure / timeout paths are code-inspected but not fully covered by tests | `coverage-gap-resolution-slice.agent.md` | `src/ToolWebGetSimpleFunctions.Functions/Services/AzureOpenAIRestExtractionClient.cs`; `tests/ToolWebGetSimpleFunctions.Functions.Tests/` |
| VK-GAP-003 | manual-only | Easy Auth / Entra real deployed behavior, allowed users/groups/app roles, unauthenticated action, and mobile browser sign-in flow cannot be verified from repository code alone | human review / deployment verification | Azure Function App Authentication settings; Entra app/users/groups/app roles |
| VK-GAP-004 | human-decision-needed | Azure Functions hosting plan, AOAI deployment/model/region/credential strategy, exact NuGet update policy, Application Insights setup remain environment decisions | human review | Azure resource configuration; deployment pipeline/IaC if added later |
...
`PARTIALLY_VERIFIED_WITH_MANUAL_GAPS`

`venueEvidence` と deterministic traceability の contract mismatch は解消され、source collector / AOAI request / validator / selector には直接テストが追加された。一方で Easy Auth の実環境確認、function entrypoint 周辺、AOAI 異常系、live source drift には manual-only または追加テストの残件がある。
...
- Decisions made:
  ...
  - `venueEvidence` / deterministic traceability mismatch is resolved in the current production code and direct tests now exist for the source collector, AOAI request builder, validator, and selector.
  - Easy Auth exact behavior is ManualOnly / NeedsHumanDecision, not automatically passable from code inspection.
  - No full `Bound` status was assigned because function entrypoint / Easy Auth / AOAI failure-path coverage is still incomplete.
...
- Remaining work:
  - `missing-test`: add function entrypoint coverage for authentication guard, date parsing, and response JSON shape.
  - `missing-test`: add AOAI malformed response / timeout / HTTP failure coverage.
...
- Recommended next step:
  - Run `coverage-gap-triage.agent.md` with this verification artifact and target gaps `VK-GAP-001` and `VK-GAP-002`.
  - For an immediate bounded fix slice, prioritize function entrypoint coverage and AOAI failure-path tests while keeping Easy Auth / live source checks as manual-only.
